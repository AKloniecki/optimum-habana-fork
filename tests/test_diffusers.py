...
class GaudiWanPipelineTester(PipelineTesterMixin, TestCase):
    """
    Tests the WAN Pipeline for Gaudi.
    """
    ...
    def get_dummy_inputs(self, device=torch_device, seed=0):
        generator = torch.Generator(device=device).manual_seed(seed)
    ...
    def test_wan_pipeline_default_case(self):
        device = torch_device
        components = self.get_dummy_components()
        wan_pipe_oh = self.pipeline_class(**components)
        wan_pipe_oh.set_progress_bar_config(disable=True)

        inputs = self.get_dummy_inputs(device)
        output = wan_pipe_oh(**inputs)
...
class GaudiWanImageToVideoPipelineTester(PipelineTesterMixin, TestCase):
    ...
    def test_wan_i2v_pipeline_default_case(self):
        device = torch_device
        components = self.get_dummy_components()
        wan_i2v_pipe = self.pipeline_class(**components)
        wan_i2v_pipe.set_progress_bar_config(disable=None)
